...
class Currency extends Number
{
    ...
    public function toMoneyInstance(mixed $value, ?string $currency = null): Money
    {
        ...
        return Money::{$method}((string) $value, $currency, $context);
    }
    ...
    public function formatMoney(mixed $value, ?string $currency = null, ?string $locale = null): string
    {
        ...
        if (\is_null($this->currencySymbol)) {
            return method_exists($money, 'formatToLocale')
                ? $money->formatToLocale($locale)
                : $money->formatTo($locale);
        }

        return tap(new NumberFormatter($locale, NumberFormatter::CURRENCY), function ($formatter) use ($money) {
